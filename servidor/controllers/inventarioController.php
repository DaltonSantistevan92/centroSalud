...
class InventarioController
{
    ...
    public function guardarIngresoProducto($id_Movimiento, $detalles = [], $tipo)
    {
        ...
        if (count($detalles) > 0) { 
            foreach ($detalles as $item) {
                ...
                $nuevo->productos_id =  intval($item->productos_id);
                $nuevo->movimientos_id = intval($id_Movimiento);
                $nuevo->tipo = $tipo;
                $nuevo->cantidad = $cantidad;

                //verifica si existe un registro anterior del producto
                $existe = Inventario::where('productos_id', intval($item->productos_id))->get()->count();
    ...
    private function tipo_inventario_first($tipo, Inventario $inventario)
    {
        ...
        if ($tipo == 'E') {
            ...
            $response = [
                'status' => false,
                'mensaje' => 'No hay productos en stock disponible, realice un abastecimiento',
            ];
        }
        return $response;
    }
    ...
    private function tipo_inventario_mas_registro($tipo, Inventario $inventario)
    {
        ...
        if ($tipo == 'S') { //salida
            $cantidad = ( $ultimo->cantidad_disponible ) + ( $inventario->cantidad ); //resta
    ...
    public function verInventario($params)
    {
        ...
        $inventario = Inventario::where('productos_id',$productos_id)->orderBy('created_at')->get();
        $data = [];  $i = 1;
